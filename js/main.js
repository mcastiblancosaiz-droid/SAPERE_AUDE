...
function initContactForm() {
  ...
  form.addEventListener('submit', function (event) {
    ...
    // Todo válido: mostramos la confirmación de demostración y reiniciamos
    // el formulario. No se envían correos ni se guarda ninguna información.
...
function initLoginForm() {
  ...
  form.addEventListener('submit', function (event) {
    ...
    // contra los usuarios guardados en localStorage (ver sección 7).
    ...
    // Credenciales correctas: se crea la sesión local y se confirma.
    createSession({
      userId: user.id,
      nombre: user.nombre,
    ...
    // Tras iniciar sesión, se redirige al panel privado del usuario.
    setTimeout(function () {
...
function initRegistroForm() {
  ...
  form.addEventListener('submit', function (event) {
    ...
    // NOTA DE SEGURIDAD: en una aplicación real la contraseña nunca se
    // guardaría en texto plano ni en el navegador del usuario (se enviaría
    // a un servidor y se almacenaría con un hash seguro, p. ej. bcrypt).
    // Aquí se guarda tal cual únicamente porque este proyecto es una
    // demostración académica sin backend.
    var newUser = {
      id: generateLocalId(),
    ...
    // No iniciamos sesión automáticamente: es más coherente que la persona
    // confirme sus credenciales una vez en login.html, igual que en un
    // flujo típico de registro. Redirigimos tras una breve pausa para que
    // el mensaje de éxito alcance a leerse.
    setTimeout(function () {
...
/* ==========================================================================
   ...
   - Las contraseñas se guardan en texto plano, lo cual NUNCA debería
     hacerse en una aplicación real (ver nota junto a initRegistroForm).
...
/** Indica si hay una sesión activa y válida (con usuario asociado). */
function isLoggedIn() {
  var session = getSession();
  return !!(session && session.loggedIn && session.userId);
}
...
/**
 * Catálogo de cursos. Mantiene los mismos cursos demostrativos que ya
 * existían en formacion.html (mismo nombre, categoría, modalidad, duración
 * y nivel); solo se les añade un id estable, una descripción de contenido
 * y una lista de módulos demostrativos coherente con el tema de cada curso.
 */
var COURSES_CATALOG = [
  {
  ...
  },
  {
    ...
    modalidad: 'Virtual',
    duracion: '10 semanas',
    descripcion: 'Resolución de problemas paso a paso, con énfasis en la interpretación de resultados.',
    contenido: 'Curso aplicado de cálculo diferencial, con resolución guiada de problemas y énfasis en la interpretación práctica de los resultados.',
    modulos: [
  ...
  },
  {
    ...
    nombre: 'Fundamentos de gestión de proyectos',
    categoria: 'Administración',
    nivel: 'Nivel básico',
    modalidad: 'Virtual',
    duracion: '5 semanas',
    descripcion: 'Introducción al ciclo de proyectos: formulación, ejecución, seguimiento y evaluación.',
    contenido: 'Introducción práctica a la gestión de proyectos, recorriendo cada etapa del ciclo: formulación, planificación, ejecución, seguimiento y evaluación.',
    modulos: [
  ...
  },
  {
    ...
    nombre: 'Fundamentos de enfermería clínica',
    categoria: 'Salud',
    nivel: 'Nivel intermedio',
    modalidad: 'Virtual',
    duracion: '12 semanas',
    descripcion: 'Conceptos clave y buenas prácticas orientadas al desempeño en contextos clínicos.',
    contenido: 'Curso orientado a conceptos clave de enfermería clínica, protocolos, buenas prácticas y procedimientos básicos en contextos de salud.',
    modulos: [
      'Conceptos clave de enfermería clínica',
      'Protocolos y buenas prácticas',
      'Evaluación de pacientes',
      'Procedimientos clínicos básicos',
      'Evaluación final'
    ]
  },
  {
    id: 'habilidades-analiticas',
    nombre: 'Habilidades analíticas para equipos',
    categoria: 'Desarrollo profesional',
    nivel: 'Todo nivel',
    modalidad: 'Virtual',
    duracion: '4 semanas',
    descripcion: 'Herramientas prácticas para estructurar el pensamiento analítico en el trabajo diario.',
    contenido: 'Curso práctico para fortalecer el pensamiento analítico en equipos de trabajo, con herramientas aplicables a la toma de decisiones diaria.',
    modulos: [
...
function initDashboardCoursesProgress() {
  ...
  if (!inscripciones.length) {
    container.innerHTML =
      ...
      '<a href="formacion.html" class="btn btn--primary">Explorar formación</a>';
    return;
  }
  ...
  container.innerHTML =
    ...
    '<div class="quick-actions"><a href="formacion.html" class="btn btn--secondary">Explorar formación</a></div>';
}
...
var RESOURCES_CATALOG = [
  ...
  },
  {
    ...
    tipo: 'PDF',
    descripcion: 'Definiciones clave de términos usados en cursos y asesorías de administración.',
...
function generarRespuestaAutomatica(texto) {
  ...
  if (/curso|formaci[oó]n|inscrib|m[oó]dulo/.test(t)) {
    return 'Puedes ver el catálogo completo e inscribirte desde "Formación", y hacer seguimiento de tu progreso en "Mis cursos".';
  }
  ...
  return 'Gracias por tu mensaje. Un/a asesor/a revisará tu solicitud pronto. Mientras tanto, puedes explorar Reservas, Formación o Recursos desde el menú.';
}
...
function saperebotBuscarCurso(texto) {
  ...
  // Coincidencias adicionales por idioma/tema frecuente, sin inventar cursos nuevos.
  if (/ingl[eé]s/.test(t)) return getCourseById('ingles-conversacional');
  if (/franc[eé]s/.test(t)) return getCourseById('frances-basico');
  if (/c[aá]lculo|matem[aá]tica/.test(t)) return getCourseById('calculo-diferencial');
  if (/proyecto|gesti[oó]n/.test(t)) return getCourseById('gestion-proyectos');
  if (/enfermer[ií]a|salud|cl[ií]nic/.test(t)) return getCourseById('enfermeria-clinica');
  if (/anal[ií]tic/.test(t)) return getCourseById('habilidades-analiticas');
  return null;
}
...
var SAPEREBOT_RESPUESTAS = {
  ...
  'que-es': function () {
    return {
      texto: [
        'SAPERE AUDE integra formación, asesoría académica y consultoría organizacional en una sola plataforma. Nació de clases de inglés y francés, y con el tiempo amplió su propuesta hacia la asesoría académica y la consultoría.',
  ...
  'formacion': function (mensaje) {
    ...
    var texto = [
      'Contamos con cursos organizados por áreas: Idiomas, Matemáticas, Ciencias sociales, Administración, Salud y Desarrollo profesional, con seguimiento de tu progreso dentro de la plataforma.',
      'Los cursos que ves actualmente son contenido de ejemplo mientras se habilita el catálogo completo.'
    ];
    if (curso) {
      texto.push('Por ejemplo, tenemos "' + curso.nombre + '" (' + curso.categoria + ', ' + curso.duracion + ').');
    }
    return { texto: texto, enlaces: [{ etiqueta: 'Ver formación', href: 'formacion.html' }] };
  },
...
var SAPEREBOT_MENU_PRINCIPAL = [
  ...
  { etiqueta: 'Formación y cursos', categoria: 'formacion' },
...
var SAPEREBOT_MENU_ORIENTACION = [
  ...
  { etiqueta: 'Quiero aprender algo nuevo', categoria: 'formacion' },
  ...
  form.addEventListener('submit', function (event) {
    ...
    for (var i = 0; i < users.length; i++) {
      if (users[i].id === user.id) {
        users[i].nombre = nombreInput.value.trim();
        users[i].apellido = apellidoInput.value.trim();
